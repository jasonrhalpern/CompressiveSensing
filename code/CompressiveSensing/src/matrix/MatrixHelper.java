...
public class MatrixHelper {
	...
	public static Matrix fillWithZeros(Matrix zeroMatrix){
		...
		for (int row = 0; row < zeroMatrix.rowSize(); row++) {
			for (int column = 0; column < zeroMatrix.columnSize(); column++) {
				/** 
				sets cell's value to zero
				*/
				zeroMatrix.set(row, column, 0);
			}
		}
	...
	*/
	...
	\param randomMatrix what is this ??? (todo)
	*/
	...
	*/
	...
	x is the zeroMatrix, v is our randomMatrix, the right side of the
	equation is the gaussMatrix
	\param zeroMatrix this is .....
	\param randomMatrix this is ....
	*/
	/*public static Matrix setCellValues(Matrix zeroMatrix, Matrix randomMatrix, 
			Matrix gaussMatrix){
	...
	*/
	...
	\param mtrx the square toor of the sum
		...
		*/
		for (int row = 0; row < numRows; row++) {
			...
			for (int column = 0; column < numColumns; column++) {
				...
				bounds check
				*/
				System.out.print(signalMatrix.get(row,column) + " ");
			}
			System.out.println();
		}
		System.out.println();
	...
	*/
	...
	\param equalityMtrx what is this input?

	\return tempMtrx linear indiced of the elements in vector ind
	*/
